...
from Models      import GCN, GNN, GAT, ParticleNetV3
...
if args.model == "GCN":
    ...
    model = GAT(nFeatures, nGraphFeatures, args.nNodes, nClasses, args.dropout_p).to(args.device)
elif args.model == "ParticleNet":
    model = ParticleNetV3(nFeatures, nGraphFeatures, nClasses, args.nNodes, args.dropout_p).to(args.device)
else:
...
def train(model, optimizer, scheduler):
    ...
    for data in trainLoader:
        out = model(data.x.to(args.device), data.edge_index.to(args.device), data.graphInput.to(args.device), batch=data.batch.to(args.device))
