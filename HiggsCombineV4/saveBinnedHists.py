...
def getHist(processName, signal, syst="Central"):
    ...
    if syst == "Central":
        h = ROOT.TH1D(f"{processName}", "", 20, mA-5*sigma, mA+5*sigma)
    else:
        h = ROOT.TH1D(f"{processName}_{syst}", "", 20, mA-5*sigma, mA+5*sigma)
...
for SIGNAL in SIGNALs:
    ...
    data_obs = ROOT.TH1D("data_obs", "", 20, mA-5*sigma, mA+5*sigma)
    print(f"@@@@ processing signal...")
