...
class DatacardManager():
    def __init__(self, era, channel, masspoint):
        self.csv = pd.read_csv(f"results/{era}/{channel}__/{masspoint}/eventRates.csv", index_col="syst")
        self.backgrounds = []
    ...
    def get_event_rate(self, process, syst="Central"):
        if syst == "stat":
            if process == "signal": return eval(self.csv.loc["Central", process])[1]*(1/3)
            else:                   return eval(self.csv.loc["Central", process])[1]
        else:
            if process == "signal": return eval(self.csv.loc[syst, process])[0]*(1/3)
    ...
    def syststring(self, syst, alias=None, type="lnN", value=None, skip=None):
        ...
        for process in ["signal"]+self.backgrounds:
            if process in skip: syststring += "-\t\t"
                ...
                if syst == "stat":
                    ratio = self.get_event_ratio(process, syst)
                else:
...
if __name__ == "__main__":
    ...
    print(manager.syststring(syst="stat", alias="norm_signal", type="lnN", skip=["nonprompt", "conversion", "diboson", "ttX", "others"]))
    print(manager.syststring(syst="stat", alias="norm_diboson", type="lnN", skip=["signal", "nonprompt", "conversion", "ttX", "others"]))
    print(manager.syststring(syst="stat", alias="norm_ttX", type="lnN", skip=["signal", "nonprompt", "conversion", "diboson", "others"]))
    print(manager.syststring(syst="stat", alias="norm_others", type="lnN", skip=["signal", "nonprompt", "conversion", "diboson", "ttX"]))
