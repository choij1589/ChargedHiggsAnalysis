...
// With MC stat uncertainty encountered

// fill methods
// method1: fill twice for each event with "mass" variable
// mththo2: only fill once for each event, store as mass1 and mass2 - for correlation study

// data field
const TString SIGNAL = "MHc-130_MA-90";     // default score value
const TString ERA = "2017";
const TString CHANNEL = "Skim3Mu";
const TString METHOD = "method1";           
...
void preprocess() {
    const TString baseOutPath = "samples/"+ERA+"/"+CHANNEL+"__/"+METHOD+"/";
...
TFile *getFile(const TString &sampleName, const bool isPrompt, const bool isSignal) {
    ...
    const TString promptPath = dataPath+"/PromptUnbinned/"+ERA+"/"+CHANNEL+"__DenseNet__";
    const TString matrixPath = dataPath+"/MatrixUnbinned/"+ERA+"/"+CHANNEL+"__DenseNet__/DATA";
    TString filePath;
    // data
