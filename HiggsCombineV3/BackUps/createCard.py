...
class DatacardManager():
    def __init__(self, era, channel, network, masspoint):
        ...
        if ("Net" in args.network): 
            self.rtfile = R.TFile.Open(f"results/{era}/{channel}__{network}__/{masspoint}/shapes_input_{args.tag}.root")
        else:
            self.rtfile = R.TFile.Open(f"results/{era}/{channel}__/{masspoint}/shapes_input_{args.tag}.root")
        self.backgrounds = []
        for bkg in ["nonprompt", "conversion", "diboson", "ttX", "others"]:
            # check if central event rates are positive
            h = self.rtfile.Get(f"{bkg}_{args.tag}")
            if not h.Integral() > 0:
    ...
    def get_event_rate(self, process, syst="Central"):
        if "data_obs" in process:
            h = self.rtfile.Get(f"data_obs_{args.tag}")
            return h.Integral()
        elif "signal" in process:
            if syst == "Central": h = self.rtfile.Get(f"{self.signal}_{args.tag}")
            else:                 h = self.rtfile.Get(f"{self.signal}_{args.tag}_{syst}")
            return h.Integral()
        else:
            if syst == "Central": h = self.rtfile.Get(f"{process}_{args.tag}")
            else:                 h = self.rtfile.Get(f"{process}_{args.tag}_{syst}")
            return h.Integral()
    ...
    def part2string(self):
        part2string = f"shapes\t*\t*\tshapes_input_{args.tag}.root\t$PROCESS_{args.tag}\t$PROCESS_{args.tag}_$SYSTEMATIC\n"
        part2string += f"shapes\tsignal\t*\tshapes_input_{args.tag}.root\t{self.signal}_{args.tag}\t{self.signal}_{args.tag}_$SYSTEMATIC\n"
        part2string += "-"*50
        return part2string
